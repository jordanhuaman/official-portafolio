    ...
    <div className='mt-[10px] flex'>
      <section className={`flex flex-col gap-1 relative ${styles.section_border}`}>
        {jobs.map((job, index) => (
          ...
          >
            <div className={`cursor-pointer hover:text-primary rounded-sm transition-all duration-[0.5s] hover:bg-[#102240] px-[30px] py-[6px] text-[14px] font-ppBold tracking-wide`}>
              {job.company}
            </div>
          </div>
        ))}
      ...
      <section className='flex-1'>
        ...
        <p className='text-primary text-[14px] font-ppMedium mb-4'>
          {currentJob.start} - {currentJob.end}
        </p>
        <ul className='flex flex-col gap-2'>
          {currentJob.tasks.map((task, index) => (
            <li key={index} className='text-slate text-[14px] flex items-start gap-10'>
