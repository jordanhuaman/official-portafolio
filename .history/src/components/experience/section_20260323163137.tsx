    ...
    <div className='flex flex-col gap-4'>
      <div>
        {jobs.map((job, index) => (
          <div
            key={index}
            className={`flex flex-row gap-10 justify-between rounded-md ${styles.section}`}
          >
            <p className={`flex-shrink-0 text-sm tracking-wide opacity-55`}>{job.start} - {job.end}</p>
            <div className=''>
              <p className='text-white text-[20px] font-semibold'>{job.position} - {job.company}</p>
